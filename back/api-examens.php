<?php
// Dedicated Examens API Endpoint
...
ini_set('error_log', __DIR__ . '/logs/examens_api_errors.log');
...
error_log(sprintf(
	"[%s] Examens API Request: Method=%s, URI=%s, Origin=%s, Headers=%s",
	date('Y-m-d H:i:s'),
...
try {
	switch ($_SERVER['REQUEST_METHOD']) {
		case 'GET':
			...
			// Handle specific examen retrieval
			if ($id) {
				$result = $examenController->getExamenById($id);
				echo json_encode($result);
				exit;
			}
			// Handle all examens
			else {
				$result = $examenController->getAllExamens();
				echo json_encode($result);
				exit;
			}
			break;
		...
		case 'POST':
			...
			if (!$data || !isset($data['titre']) || !isset($data['matiere']) || !isset($data['classe']) || !isset($data['date'])) {
				...
				echo json_encode([
					'success' => false,
					'message' => 'Missing required fields (titre, matiere, classe, date)'
				]);
				exit;
			}
		...
		case 'PUT':
			...
			// Validate input
			if (!$data || !isset($data['id']) || !isset($data['titre']) || !isset($data['matiere']) || !isset($data['classe']) || !isset($data['date'])) {
				http_response_code(400);
				echo json_encode([
					'success' => false,
					'message' => 'Missing required fields (id, titre, matiere, classe, date)'
				]);
				exit;
			}
		...
		case 'DELETE':
			...
			// Delete the examen
			$result = $examenController->deleteExamen($data['id']);
			echo json_encode($result);
			exit;
			break;
	...
	error_log("Examens API error: " . $e->getMessage());
	http_response_code(500);
	echo json_encode([
		'success' => false,
