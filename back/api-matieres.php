<?php
// Dedicated Matieres API Endpoint
...
ini_set('error_log', __DIR__ . '/logs/matieres_api_errors.log');
...
error_log(sprintf(
	"[%s] Matieres API Request: Method=%s, URI=%s, Origin=%s, Headers=%s",
	date('Y-m-d H:i:s'),
...
try {
	switch ($_SERVER['REQUEST_METHOD']) {
		case 'GET':
			...
			// Handle specific matiere retrieval
			if ($id) {
				$result = $matiereController->getMatiereById($id);
		...
		case 'POST':
			...
			if (!$data || !isset($data['nom'])) {
				...
				echo json_encode([
					'success' => false,
					'message' => 'Missing required fields (nom)'
				]);
				exit;
			}
		...
		case 'PUT':
			...
			if (!$data || !isset($data['id']) || !isset($data['nom'])) {
				...
				echo json_encode([
					'success' => false,
					'message' => 'Missing required fields (id, nom)'
				]);
				exit;
			}

			// Update the matiere
			$result = $matiereController->updateMatiere($data['id'], $data);
			echo json_encode($result);
			exit;
			break;
		...
		case 'DELETE':
			...
			// Delete the matiere
			$result = $matiereController->deleteMatiere($data['id']);
			echo json_encode($result);
			exit;
			break;
	...
	error_log("Matieres API error: " . $e->getMessage());
	http_response_code(500);
	echo json_encode([
		'success' => false,
